...
5. ~~Should `web_search` results be capped by total characters as well as `maxResults`?~~ **Decided in SR-059: yes, 4,000 characters**, counting titles and urls as well as snippets, because all three are text a stranger wrote arriving inside the agent's context. The hit that crosses the line is truncated rather than dropped, so a partial extract still carries a link worth following.
