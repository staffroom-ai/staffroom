...
| `ClientMessage` and `ServerMessage` unions (every WebSocket type: `hello`, `task.create`, `task.cancel`, `chat.send`, `approval.decide`, `agent.rename`, `provider.set_key`, `routine.*`, `brain.search`, `brain.graph.get`, `runs.replay`, `mcp.reconnect`, `mcp.oauth.begin`, `demo.speed`, `office.reload`, `ping`; `welcome`, `ack`, `error`, `state`, `event`, `replay`, `config.error`, `config.reloaded`, `tools.reloaded`, `mcp.status`, `mcp.tools_changed`, `mcp.oauth.url`, `brain.results`, `brain.graph`, `brain.note.indexed`, `brain.note.removed`, `brain.warning`, `pong`) | protocol | `server-cli-runtime.md` §6 |
